...
if (!isset($_GET['id'])) {
    header('Location: /future/science');
    exit;
} 
...
if($site_info['enableCaching']=="1"){
    ...
    if (!$CachedString->isHit()) {
        ...
        $news_detail_data = loadDate($commonData['menus_top'],$lang,$id);
    ...
    $result =  loadDate($commonData['menus_top'],$lang,$id);
...
function loadDate($menus,$lang,$id){
    ...
    if(isset($data)){
        ...
        $teams = Team::where('active',1)->whereIn('id',$authors)->where('lang',$lang)->orderBy('importance','DESC')->get();
    ...
    return  ['article' => $data, 'subnavs' => $subnavs, 'metadata'=>$metadata,'carousel'=>$carousel,'authors'=>$teams];
}

$uri = $lang=='zh-CN'?'/future/science':'/'.$lang.'/future/science';
...
echo $twig->render('future/detail.html', $result);
