...
function loadDate($menus,$lang){
    ...
    $curent_url = "/{$lang}/development";
    $parent = $menus->where('url',$curent_url)->first();
    $subnavs = isset($parent) ? $parent->children : null;
    $current_menu = isset($subnavs) ? $subnavs->where('url',$curent_url.'/sustainability')->first():null;
    ...
    $metadata = Metadata::where('module_type',ModuleType::URL())->where('key_value', $curent_url.'/sustainability')->first();

    return  ['page'=>$data,'articles' => $articles, 'subnavs'=>$subnavs, 'current_menu'=>$current_menu, 'metadata'=>$metadata,'carousel'=>$carousel];
}
...
echo $twig->render('development/sustainability.twig', $result);
