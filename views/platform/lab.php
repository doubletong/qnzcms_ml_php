...
function loadDate($lang,$menus){
    ...
    $labs = Lab::where('active',1)->where('lang',$lang)->orderBy('importance','DESC')->get();

    if($lang == 'zh-CN'){   
        $subnavs = $menus->where('url','/platform')->first()->children;
        $metakey = '/platform/lab';  
    }else{     
        $subnavs = $menus->where('url','/'.$lang.'/platform')->first()->children;
        $metakey = '/'.$lang.'/platform/lab';  
    }
    ...
    return  ['subnavs' => $subnavs, 'metadata'=>$metadata,'carousel'=>$carousel,'labs'=>$labs];
}
...
echo $twig->render('platform/lab.html', $result);
