...
if($site_info['enableCaching']=="1"){
    ...
    if (!$CachedString->isHit()) {          
        ...
        $news_detail_data = loadDate($commonData['menus_bot'], $query,$itemsPerPage,$currentPage,$cid,$lang);
    ...
    $result =  loadDate($commonData['menus_bot'],$query,$itemsPerPage,$currentPage,$cid,$lang);
...
//load data
function loadDate($menus, $query,$itemsPerPage,$currentPage,$cid,$lang){
    ...
    $catelistvm = array();
    $categories = NewsCategory::where('active',1)->where('parent',15)->orderBy('importance', 'DESC')->get();
    ...
    $urlPattern = $lang=='zh-CN'? '/news/notice/c'.$cid.'/page-(:num)': '/'.$lang.'/news/notice/c'.$cid.'/page-(:num)';
...
echo $twig->render('news/notice.html', $result);
